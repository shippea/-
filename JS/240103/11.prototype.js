...
$.get({
    ...
    success: function(data){
        ...
        // 객체들을 반복하면서 Member 객체를 생성
        for (let eachObj of valueArray) {

            const mpart = eachObj.mpart;
            const memail = eachObj.memail;
            new Member(eachObj.mtype, eachObj.mid, eachObj.mname);
            ...
            if (mpart) {
                ...
                user.memail = eachObj.memail;
                resultArray[resultArray.length] = user;
            }
            // ul의 li에 생성된 Member 객체들을 출력
