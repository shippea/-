...
RETURN varchar2
IS 
	f_max_name varchar2(100);
	f_min_name varchar2(100);
	f_max_sal EMPLOYEES.SALARY%TYPE;
	f_min_sal EMPLOYEES.SALARY%TYPE;
	f_did EMPLOYEES.DEPARTMENT_ID%TYPE;
BEGIN 
	SELECT e.DEPARTMENT_ID
		INTO f_did
	...
	SELECT e.FIRST_NAME||' '||e.LAST_NAME
		...
		FROM EMPLOYEES e
		WHERE e.DEPARTMENT_ID = f_did
			AND e.SALARY = f_max_sal;
	SELECT e.FIRST_NAME||' '||e.LAST_NAME
		INTO f_min_name
		FROM EMPLOYEES e
		WHERE e.DEPARTMENT_ID = f_did
			AND e.SALARY = f_min_sal;
	RETURN '최고급여자: '||f_max_name||', 최고급여: '||f_max_sal||', 최소급여자: '||f_min_name||', 최소급여: '||f_min_sal; 
END;

BEGIN
	DBMS_OUTPUT.PUT_LINE(FUNC_MAXMINSALARY(105));
END;
