...
--------------------------------------------------------------------
SELECT (SELECT
		CASE WHEN e2.SALARY >=10000 THEN '고소득자'
			WHEN e2.SALARY >=5000 AND e2.SALARY<10000 THEN '일반소득자'
			ELSE '저소득자'
			END AS class), e1.*
...
SELECT *
FROM DEPARTMENTS d
WHERE to_char(d.DEPARTMENT_NAME);
...
SELECT TO_TIMESTAMP_TZ(jh.START_DATE) 
FROM JOB_HISTORY jh;
...
SELECT d.DEPARTMENT_NAME, e.DEPARTMENT_ID, 
	sum(e.SALARY*(1+
	CASE WHEN e.COMMISSION_PCT IS NULL THEN 0.1 ELSE e.COMMISSION_PCT END))
FROM EMPLOYEES e, DEPARTMENTS d
WHERE e.DEPARTMENT_ID = d.DEPARTMENT_ID
